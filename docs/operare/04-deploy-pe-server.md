...
| Unde | Ce |
|------|----|
| `.env.prod` → `BASE_URL`, `FRONTEND_ORIGIN` | `http://SUBDOMENIU` (apoi `https://` după cert) |
| `deploy/nginx-proxy/statistic.conf` → `server_name` + căile certului | `SUBDOMENIU` real |
| DNS | A-record `SUBDOMENIU` → IP-ul serverului |
...
```bash
# editează BASE_URL / FRONTEND_ORIGIN cu http://SUBDOMENIU
...
sed -i 's/app.exemplu.ro/SUBDOMENIU/g' /root/nginx-proxy/conf.d/statistic.conf
...
```
Verifică pe HTTP: `curl -I http://SUBDOMENIU/health` → 200. Deschide `http://SUBDOMENIU` în browser și loghează-te (admin din `.env.prod`).
...
docker run --rm \
  ...
  -d SUBDOMENIU --email EMAILUL_TAU --agree-tos --no-eff-email
```
...
docker exec nginx_proxy nginx -t && docker exec nginx_proxy nginx -s reload
```
Apoi în `.env.prod`: `BASE_URL`/`FRONTEND_ORIGIN` → `https://SUBDOMENIU`, `COOKIE_SECURE=true`, și reîncarcă backend-ul:
```bash
docker compose -f docker-compose.prod.yml --env-file .env.prod up -d backend
```
Verifică: `https://SUBDOMENIU` se deschide cu lacăt, login funcționează.
...
## Depanare
- `curl http://SUBDOMENIU/health` merge dar browserul nu → verifică DNS (A-record → IP).
